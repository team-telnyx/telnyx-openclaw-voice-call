...
function createAgentRuntime(payloads: Array<Record<string, unknown>>) {
  ...
  const resolveAgentWorkspaceDir = vi.fn(
    (_cfg: CoreConfig, agentId: string) => {
...
function requireEmbeddedAgentArgs(
  ...
  if (!firstCall) {
    throw new Error(
      "voice response generator did not invoke the embedded agent",
    );
  }
  ...
  if (!args?.extraSystemPrompt) {
    throw new Error(
      "voice response generator did not pass the spoken-output contract prompt",
    );
  }
  return args;
}
...
describe("generateVoiceResponse", () => {
  ...
  it("pins the voice session to responseModel before running the embedded agent", async () => {
    const { runtime, runEmbeddedPiAgent, updateSessionStore, sessionStore } =
