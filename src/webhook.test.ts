...
const createConfig = (
  ...
  const merged = {
    ...
    realtime: {
      ...
      fastContext: {
        ...
        sources:
          overrides.realtime?.fastContext?.sources ??
          base.realtime.fastContext.sources,
      },
      agentContext: {
        ...base.realtime.agentContext,
        ...overrides.realtime?.agentContext,
        files:
          overrides.realtime?.agentContext?.files ??
          base.realtime.agentContext.files,
      },
  ...
  const parsed = VoiceCallConfigSchema.parse({
    ...
    serve: {
      ...merged.serve,
      port: merged.serve.port === 0 ? 1 : merged.serve.port,
    },
  });
...
function hasPort(value: unknown): value is { port: number | string } {
  ...
  return (
    typeof maybeAddress.port === "number" ||
    typeof maybeAddress.port === "string"
  );
}

function requireBoundRequestUrl(
...
describe.skip("VoiceCallWebhookServer realtime transcription provider selection (needs Telnyx rewrite)", () => {
  it("auto-selects the first registered provider when streaming.provider is unset", async () => {
    ...
    try {
      ...
      expect(mocks.listRealtimeTranscriptionProviders).toHaveBeenCalledWith(
        null,
      );
  ...
  const resolveMediaStreamClientIp = (
    ...
    return (
      server as unknown as {
        resolveMediaStreamClientIp: (
          request: MediaStreamRequestDouble,
        ) => string | undefined;
      }
...
async function runStaleCallReaperCase(params: {
  ...
  const call = {
    ...createCall(now.getTime() - params.callAgeMs),
...
describe("VoiceCallWebhookServer path matching", () => {
  it("rejects lookalike webhook paths that only match by prefix", async () => {
    const verifyWebhook = vi.fn(() => ({
      ok: true,
      verifiedRequestKey: "verified:req:prefix",
    }));
...
describe.skip("VoiceCallWebhookServer pre-auth webhook guards (needs Telnyx rewrite)", () => {
  ...
  it.skip("limits concurrent pre-auth requests per source IP", async () => {
    ...
    try {
      ...
      const rejected = await postWebhookFormWithHeaders(
        server,
        baseUrl,
        "CallSid=CA999",
        headers,
...
describe.skip("VoiceCallWebhookServer stream disconnect grace (needs Telnyx rewrite)", () => {
  ...
  it.skip("ignores stale stream disconnects after reconnect and only hangs up on current stream disconnect", async () => {
    ...
    if (!mediaHandler) {
      throw new Error(
        "expected webhook server to expose a media stream handler",
      );
    }
...
describe.skip("VoiceCallWebhookServer barge-in suppression during initial message (needs Telnyx rewrite)", () => {
  ...
  it.skip("suppresses barge-in clear while outbound conversation initial message is pending", async () => {
    ...
    try {
      ...
      expect(processEvent).toHaveBeenCalledTimes(1);
      const [calledCallId, calledTranscript] = (handleInboundResponse.mock
        .calls[0] ?? []) as unknown as [string | undefined, string | undefined];
